...
# this is key factor in some situation
torch.manual_seed(1)
...
def prepare_sequence(sequence: Tensor, to_ix: Dict[int, float]):
    idxs = [to_ix[w] for w in sequence]
    return torch.tensor(idxs, dtype=torch.long)


# Compute log sum exp in a numerically stable way for the forward algorithm
def log_sum_exp(tensor: Tensor):
    max_score = tensor[0, argmax(tensor)]
    max_score_broadcast = max_score.view(1, -1).expand(1, tensor.size()[1])
    return max_score + \
        torch.log(torch.sum(torch.exp(tensor - max_score_broadcast)))
